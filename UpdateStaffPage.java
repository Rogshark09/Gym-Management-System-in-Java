...
class UpdateStaffF extends JFrame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource()==b3)
		{
			try
			{
				...
				ResultSet rs=st.executeQuery("select * from staff_table");
				int flag=0;
				System.out.println("1");
				while(rs.next())
				{
					System.out.println("2");
					Staff_Id=rs.getInt(3);
				...
				if(flag==1)
				{
					JOptionPane.showMessageDialog(f1,"Staff Member Record Found");
				}
				else
				{
					JOptionPane.showMessageDialog(f1,"Staff Member record not found");
				}
				st.close();
				con.close();
			}
			...
			if(ae.getSource()==b1)
			{
				...
				Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/project","root","");
				Statement st=con.createStatement();
				int Staff_Id=Integer.parseInt(tf2.getText());
				String a="'"+tf3.getText()+"'";
				String b="'"+tf4.getText()+"'";
				String c=tf5.getText();
				String q="update staff_table set name="+a+",address="+b+",contact="+c+" where staff_id="+Staff_Id+"";
				System.out.println(q);
				st.executeUpdate(q);
				JFrame f2=new JFrame();
				JOptionPane.showMessageDialog(f2," Staff Member record updated");
				setVisible(false);
				new HomepageF();
				st.close();
				con.close();
